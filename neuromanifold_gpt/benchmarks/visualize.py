...
def plot_sample_quality(quality_results: Dict, output_path: Path):
    ...
    bars1 = ax.bar(x - width/2, standard_scores, width, label='Standard',
                   color='#3498db')
    bars2 = ax.bar(x + width/2, neuromanifold_scores, width, label='NeuroManifold',
                   color='#e74c3c')
...
def plot_speed_summary(speed_results: Dict, output_path: Path):
    ...
    ax.text(0.5, 0.6, f"Speed Benchmark: {status.upper()}",
            ha='center', va='center', fontsize=16, fontweight='bold',
            color='green' if status == 'completed' else 'red')
    ax.text(0.5, 0.4, note,
...
def plot_memory_summary(memory_results: Dict, output_path: Path):
    ...
    ax.text(0.5, 0.6, f"Memory Benchmark: {status.upper()}",
            ha='center', va='center', fontsize=16, fontweight='bold',
            color='green' if status == 'completed' else 'red')
    ax.text(0.5, 0.4, note,
            ha='center', va='center', fontsize=11, style='italic',
...
def plot_results(
    ...
    output_dir: Union[str, Path] = "benchmark_plots"
):
