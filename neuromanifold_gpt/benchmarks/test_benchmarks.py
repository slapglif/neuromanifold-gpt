...
def test_standard_config_structure():
    ...
    from neuromanifold_gpt.config.benchmarks.standard_attention import config, out
    ...
    assert not out["use_sdr"]
    assert not out["use_kan"]
    assert not out["use_mhc"]
    assert not out["use_mtp"]
    assert not out["use_fhn_parallel"]
...
def test_neuromanifold_config_structure():
    ...
    from neuromanifold_gpt.config.benchmarks.neuromanifold_attention import config, out
...
def test_run_all_cli():
    ...
    run_all.parser if hasattr(run_all, "parser") else argparse.ArgumentParser()
