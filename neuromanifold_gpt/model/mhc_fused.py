...
def mhc_width_forward_kernel(
    ...
    residuals_stride_bs, residuals_stride_t, residuals_stride_d,
    # Strides for outputs
    residuals_out_stride_bs, residuals_out_stride_t, residuals_out_stride_d,
    branch_input_stride_b, branch_input_stride_t, branch_input_stride_d,
    # Strides for H matrices
    h_res_stride_0, h_res_stride_1,
    h_pre_stride_0, h_pre_stride_1,
    ...
    for s_in in range(S):
        ...
        residual_idx = (b * S + s_in) * residuals_stride_bs + t * residuals_stride_t + d * residuals_stride_d
    ...
    branch_out_idx = b * branch_input_stride_b + t * branch_input_stride_t + d * branch_input_stride_d
    ...
    for s_out in range(S):
        ...
        residual_out_idx = (b * S + s_out) * residuals_out_stride_bs + t * residuals_out_stride_t + d * residuals_out_stride_d
...
def mhc_width_backward_kernel(
    ...
    # Strides
    residuals_stride_bs, residuals_stride_t, residuals_stride_d,
    grad_residuals_out_stride_bs, grad_residuals_out_stride_t, grad_residuals_out_stride_d,
    grad_branch_input_stride_b, grad_branch_input_stride_t, grad_branch_input_stride_d,
    grad_residuals_stride_bs, grad_residuals_stride_t, grad_residuals_stride_d,
    h_res_stride_0, h_res_stride_1,
    h_pre_stride_0, h_pre_stride_1,
    BLOCK_SIZE: tl.constexpr
):
    ...
    grad_branch_idx = b * grad_branch_input_stride_b + t * grad_branch_input_stride_t + d * grad_branch_input_stride_d
    grad_branch_val = tl.load(grad_branch_input_ptr + grad_branch_idx, mask=mask, other=0.0)
    ...
    for s in range(S):
        ...
        residual_idx = (b * S + s) * residuals_stride_bs + t * residuals_stride_t + d * residuals_stride_d
        residual_val = tl.load(residuals_ptr + residual_idx, mask=mask, other=0.0)

        # Accumulate grad_residuals[b, t, s, d]
        grad_res_val = h_pre_val * grad_branch_val
        grad_res_idx = (b * S + s) * grad_residuals_stride_bs + t * grad_residuals_stride_t + d * grad_residuals_stride_d
    ...
    for s_out in range(S):
        ...
        grad_res_out_idx = (b * S + s_out) * grad_residuals_out_stride_bs + t * grad_residuals_out_stride_t + d * grad_residuals_out_stride_d
        grad_res_out_val = tl.load(grad_residuals_out_ptr + grad_res_out_idx, mask=mask, other=0.0)
        ...
        for s_in in range(S):
            ...
            grad_res_idx = (b * S + s_in) * grad_residuals_stride_bs + t * grad_residuals_stride_t + d * grad_residuals_stride_d
            tl.atomic_add(grad_residuals_ptr + grad_res_idx, grad_res_val, mask=mask)
...
class FusedMHCWidthConnection(torch.autograd.Function):
    ...
    def forward(ctx, residuals, h_res, h_pre):
        ...
        branch_input = torch.empty(B, T, D, device=residuals.device, dtype=residuals.dtype)
        ...
        mhc_width_forward_kernel[grid](
            ...
            residuals_out, branch_input,
            B, T, D, S,
            # Residuals strides (B*S, T, D)
            residuals.stride(0), residuals.stride(1), residuals.stride(2),
            # Residuals_out strides (B*S, T, D)
            residuals_out.stride(0), residuals_out.stride(1), residuals_out.stride(2),
            # Branch_input strides (B, T, D)
            branch_input.stride(0), branch_input.stride(1), branch_input.stride(2),
            # H_res strides (S, S)
            h_res.stride(0), h_res.stride(1),
            # H_pre strides (1, S)
    ...
    def backward(ctx, grad_branch_input, grad_residuals_out):
        ...
        mhc_width_backward_kernel[grid](
            grad_residuals_out, grad_branch_input,
            residuals, h_res, h_pre,
            grad_residuals, grad_h_res, grad_h_pre,
            B, T, D, S,
            # Residuals strides
            residuals.stride(0), residuals.stride(1), residuals.stride(2),
            # Grad residuals_out strides
            grad_residuals_out.stride(0), grad_residuals_out.stride(1), grad_residuals_out.stride(2),
            # Grad branch_input strides
            grad_branch_input.stride(0), grad_branch_input.stride(1), grad_branch_input.stride(2),
            # Grad residuals strides
            grad_residuals.stride(0), grad_residuals.stride(1), grad_residuals.stride(2),
            # H_res strides
            h_res.stride(0), h_res.stride(1),
            # H_pre strides
            h_pre.stride(0), h_pre.stride(1),
