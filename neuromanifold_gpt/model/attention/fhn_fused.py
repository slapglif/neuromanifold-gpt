...
def fhn_imex_backward_kernel(
    ...
    grad_u_out_ptr,
    grad_w_out_ptr,  # Incoming grads (from next layer)
    u_ptr,
    w_ptr,
    input_ptr,  # Saved states (from forward)
    grad_u_in_ptr,
    grad_w_in_ptr,
    grad_input_ptr,  # Outgoing grads (to prev layer)
    ...
    tl.load(w_ptr + offsets, mask=mask)
...
class FusedFHNSolver(torch.autograd.Function):
    ...
    def forward(ctx, u, w, inputs, n_steps, threshold, tau, dt):
        ...
        fhn_imex_kernel_op[grid](
            ...
            inputs,
            u_out,
            w_out,  # New output pointers
            dt_tensor,  # scalar dt POINTER
            tau,
            threshold,  # other constants
            n_elements,
