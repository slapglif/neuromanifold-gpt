...
try:
    kaufmann_reaction_diffusion_step = torch.compile(
        _kaufmann_reaction_diffusion_step, mode="reduce-overhead"
    )
...
class KaufmannAttention(nn.Module):
    ...
    def __init__(self, embed_dim, n_heads, config=None, **kwargs):
        ...
        fhn_threshold = getattr(
            config, "fhn_threshold", kwargs.get("fhn_threshold", 0.5)
        )
        ...
        self.knot_gate = KnotAttention(
            embed_dim, manifold_dim=manifold_dim, n_heads=n_heads
        )
