...
def karmarkar_karp_partition(values: torch.Tensor, k_partitions: int = 2) -> torch.Tensor:
    """
    ...
    N = values.shape[-1]
    assignment = torch.zeros_like(indices, dtype=torch.long)
...
class SpectralPartitioner(nn.Module):
    ...
    def forward(self, spectral_input: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """
    ...
    def unfold(self, folded_output: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
        """
