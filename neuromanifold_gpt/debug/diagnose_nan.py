...
encoder = SemanticFoldingEncoder(vocab_size, sdr_size, n_active=20, embed_dim=64).to(device)
...
attn = FHNAttention(embed_dim, n_heads, threshold=0.5, tau=12.5, n_fhn_steps=2).to(device)
    ...
    for name, param in model.named_parameters():
        if param.grad is not None:
            ...
            if grad_info['has_nan'] or grad_info['has_inf']:
                print(f"   ❌ {name}: NaN={grad_info['has_nan']}, Inf={grad_info['has_inf']}")
else:
...
est_4layer = timings["SemanticFolding"] + 4 * (timings["ManifoldProjection"] + timings["SpectralDecomposition"] + timings["FHNAttention"])
