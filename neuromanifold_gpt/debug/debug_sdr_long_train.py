...
def main():
    ...
    config = NeuroManifoldConfig(
        ...
        n_heads=4,
        n_embd=256,
        sdr_size=2048,  # Larger SDR
        sdr_sparsity=0.05,  # 5% = 102 active bits (vs 2% = 20 bits)
        sdr_embed_dim=256,  # Match n_embd
        sdr_context_size=8,  # Larger context window
        manifold_dim=32,
        n_eigenvectors=16,
        use_sdr=True,
        use_kan=True,
    ...
    with torch.no_grad():
        ...
        logger.info(f"Mean prediction entropy: {entropy:.4f}")
