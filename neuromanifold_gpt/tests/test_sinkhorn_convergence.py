...
class TestSinkhornConvergence:
    ...
    def test_backward_compatibility_no_convergence_tol(self):
        ...
        assert torch.allclose(
            col_sums, torch.ones_like(col_sums), atol=1e-4
    ...
    def test_early_stopping_triggers(self):
        ...
        logits = (
            torch.eye(8, device=device) * 2.0 - 8.0
        )  # diagonal = 2, off-diagonal = -8
        ...
        result = sinkhorn_log(
            logits, num_iters=num_iters, convergence_tol=convergence_tol
        )
    ...
    def test_iteration_reduction_with_well_initialized_matrix(self):
        ...
        assert torch.allclose(
            row_sums_no, torch.ones_like(row_sums_no), atol=1e-4
        ), "No-early rows don't sum to 1"
        assert torch.allclose(
            col_sums_no, torch.ones_like(col_sums_no), atol=1e-4
        ), "No-early columns don't sum to 1"
        assert torch.allclose(
            row_sums_early, torch.ones_like(row_sums_early), atol=1e-4
        ), "Early-stop rows don't sum to 1"
        assert torch.allclose(
            col_sums_early, torch.ones_like(col_sums_early), atol=1e-4
        ), "Early-stop columns don't sum to 1"

        # Results should be close (both converged)
        assert torch.allclose(
            result_no_early, result_with_early, atol=1e-4
    ...
    def test_different_convergence_tolerances(self):
        ...
        for i in range(len(results) - 1):
            assert torch.allclose(
                results[i], results[i + 1], atol=1e-3
    ...
    def test_extreme_convergence_case(self):
        ...
        assert (
            diagonal_values > 0.5
        ).all(), "Diagonal not dominant (expected near-identity)"
