...
class TestComputeStateStatistics:
    ...
    def test_state_statistics_keys(self):
        ...
        expected_keys = {
            'fhn_state_mean', 'fhn_state_std',
            'fhn_state_min', 'fhn_state_max',
            'fhn_state_range'
        }
...
class TestComputePulseWidthStatistics:
    ...
    def test_pulse_width_statistics_keys(self):
        ...
        expected_keys = {
            'pulse_width_mean', 'pulse_width_std',
...
class TestComputeAll:
    ...
    def test_compute_all_complete_info(self):
        ...
        expected_keys = {
            'fhn_state_mean', 'fhn_state_std',
            'fhn_state_min', 'fhn_state_max', 'fhn_state_range',
            'pulse_width_mean', 'pulse_width_std',
            'pulse_width_min', 'pulse_width_max',
            'fhn_stability_bounded', 'fhn_state_abs_mean'
        }
    ...
    def test_compute_all_scalar_tensors(self):
        ...
        info = {
            'fhn_state': torch.tensor(0.5),
            'pulse_widths': torch.tensor(4.5)
        }
        metrics = FHNMetrics.compute_all(info)
...
class TestEdgeCases:
    ...
    def test_none_values_in_info(self):
        ...
        info = {
            'fhn_state': None,
            'pulse_widths': torch.randn(10).abs() + 2.0
        }
        metrics = FHNMetrics.compute_all(info)
...
class TestIntegrationWithFHNAttention:
    ...
    def test_multi_layer_aggregation(self):
        ...
        avg_pulse_width = sum(info['pulse_width_mean'] for info in layer_infos) / len(layer_infos)
        avg_stability = sum(info['fhn_stability_bounded'] for info in layer_infos) / len(layer_infos)
