...
class OptunaHPO:
    ...
    def export_best_config(self, output_path: str) -> None:
        ...
        for section_name, param_names in param_groups.items():
            ...
            section_params = [(k, v) for k, v in best_params.items() if k in param_names]
        ...
        remaining_params = [(k, v) for k, v in sorted(best_params.items()) if k not in defined_params]
        if remaining_params:
