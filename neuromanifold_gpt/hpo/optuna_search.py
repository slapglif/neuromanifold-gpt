...
class OptunaHPO:
    ...
    def _create_study(self) -> optuna.Study:
        ...
        if sampler_name == "tpe":
            ...
            logger.warning(f"Unknown sampler '{sampler_name}', defaulting to TPE")
            sampler = optuna.samplers.TPESampler(seed=1337)
    ...
    def _objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            ...
            return (
                float("inf")
                if self.study_config["direction"] == "minimize"
                else float("-inf")
            )
    ...
    def _create_train_config(
        ...
        out_dir = os.path.join(
            params.get("out_dir", "out-hpo"), f"trial_{trial_number:04d}"
        )
    ...
    def _train_trial(self, train_config: TrainConfig, trial: optuna.Trial) -> float:
        ...
        if pruning_config.get(
            "enabled", True
        ):  # Default to enabled if pruning config exists
    ...
    def optimize(self, n_trials: Optional[int] = None) -> optuna.Study:
        ...
        logger.info(
            f"Search space: {len(self.search_space.get_param_names())} parameters"
        )
        logger.info(
            f"Fixed parameters: {len(self.search_space.get_fixed_param_names())}"
        )
        ...
        # Export best configuration to config file
        output_path = self.study_config.get("export_path", "config/hpo_best.py")
        self.export_best_config(output_path)
    ...
    def export_best_config(self, output_path: str) -> None:
        ...
        Exports the best hyperparameter configuration in the same format as
        config/ralph_iter*.py files, with parameters organized into logical
        sections (Data, Model, Training, etc.).
        ...
        # Define parameter groups following ralph_iter*.py format
        param_groups = {
            "Data": [
                "dataset",
                "batch_size",
        ...
        lines = [
            f"# Best HPO Configuration - Trial {self.study.best_trial.number}",
            f"# Validation loss: {self.study.best_value:.4f}",
            "# Generated by Optuna HPO",
            "",
        ]

        # Add parameters grouped by section
        for section_name, param_names in param_groups.items():
            # Check if any parameters in this section exist
            section_params = [
                (k, v) for k, v in best_params.items() if k in param_names
            ]
            ...
            # Add parameters in the order defined
            for param_name in param_names:
                if param_name in best_params:
                    value = best_params[param_name]
                    lines.append(self._format_param(param_name, value))

            # Add blank line after section
            lines.append("")

        # Add any remaining parameters not in defined groups
        defined_params = set()
        for param_names in param_groups.values():
            defined_params.update(param_names)

        remaining_params = [
            (k, v) for k, v in sorted(best_params.items()) if k not in defined_params
        ]
        if remaining_params:
    ...
    def _format_param(self, key: str, value: Any) -> str:
        """Format a parameter as a Python assignment statement.
