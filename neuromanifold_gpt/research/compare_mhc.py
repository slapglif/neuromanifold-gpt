...
def train_and_eval(config, name, data, stoi, itos, device, n_iters=500):
    ...
    start_tokens = torch.tensor(
        [[stoi["\n"], stoi["K"], stoi["I"], stoi["N"], stoi["G"], stoi[":"]]],
        device=device,
    )
...
def main():
    ...
    configs = [
        ...
        (
            NeuroManifoldConfig(**{**base, "use_mhc": True, "use_full_mhc": False}),
            "simplified_mhc",
        ),
        (
            NeuroManifoldConfig(
                **{**base, "use_mhc": True, "use_full_mhc": True, "mhc_n_streams": 2}
            ),
            "full_mhc_2",
        ),
    ]
    ...
    for config, name in configs:
        ...
        logger.info(
            f"Result: loss={result['final_loss']:.4f}, ppl={result['perplexity']:.2f}"
        )
    ...
    for r in sorted(results, key=lambda x: x["perplexity"]):
        logger.info(
            f"{r['name']}: ppl={r['perplexity']:.2f}, loss={r['final_loss']:.4f}, params={r['params']:,}"
        )
