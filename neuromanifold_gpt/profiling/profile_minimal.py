...
def main():
    ...
    r = profile_component("SDR_Projection", sdr_proj, lambda: (torch.randn(BATCH_SIZE, SEQ_LEN, sdr_size, device=DEVICE),), n_warmup=2, n_iters=5, device=DEVICE, track_memory=True)
    results.append(r)
    del sdr_proj
    cleanup()
    ...
    r = profile_component("ManifoldProjection", manifold, lambda: (torch.randn(BATCH_SIZE, SEQ_LEN, sdr_size, device=DEVICE),), n_warmup=2, n_iters=5, device=DEVICE, track_memory=True)
    results.append(r)
    del manifold
    cleanup()
    ...
    r = profile_component("SpectralDecomp", spectral, lambda: (torch.randn(BATCH_SIZE, SEQ_LEN, manifold_dim, device=DEVICE), None), n_warmup=2, n_iters=5, device=DEVICE, track_memory=True)
    results.append(r)
    del spectral
    cleanup()
    ...
    r = profile_component("FHNDynamics", fhn, lambda: (torch.randn(BATCH_SIZE, n_heads, n_eigenvectors, head_dim, device=DEVICE), 2), n_warmup=2, n_iters=5, device=DEVICE, track_memory=True)
    results.append(r)
    del fhn
    cleanup()
    ...
    r = profile_component("FHNAttention", fhn_attn, lambda: (torch.randn(BATCH_SIZE, SEQ_LEN, embed_dim, device=DEVICE), torch.randn(BATCH_SIZE, SEQ_LEN, n_eigenvectors, device=DEVICE)), n_warmup=2, n_iters=5, device=DEVICE, track_memory=True)
    results.append(r)
    del fhn_attn
    cleanup()
    ...
    r = profile_component("WaveKAN_FFN", wavekan, lambda: (torch.randn(BATCH_SIZE, SEQ_LEN, embed_dim, device=DEVICE),), n_warmup=2, n_iters=5, device=DEVICE, track_memory=True)
    results.append(r)
    del wavekan
    cleanup()
    ...
    r = profile_component("SwiGLU_FFN", swiglu, lambda: (torch.randn(BATCH_SIZE, SEQ_LEN, embed_dim, device=DEVICE),), n_warmup=2, n_iters=5, device=DEVICE, track_memory=True)
    results.append(r)
    del swiglu
    cleanup()
    ...
    r = profile_component("ChebyKAN_FFN", chebykan, lambda: (torch.randn(BATCH_SIZE, SEQ_LEN, embed_dim, device=DEVICE),), n_warmup=2, n_iters=5, device=DEVICE, track_memory=True)
    results.append(r)
    del chebykan
    cleanup()
    ...
    r = profile_component("NeuroManifoldBlock", block, lambda: (torch.randn(BATCH_SIZE, SEQ_LEN, sdr_size, device=DEVICE),), n_warmup=2, n_iters=5, device=DEVICE, track_memory=True)
    results.append(r)
    del block
    cleanup()
    ...
    r = profile_component("SemanticFoldingEncoder", encoder, lambda: (torch.randint(0, 65, (BATCH_SIZE, SEQ_LEN), device=DEVICE),), n_warmup=2, n_iters=5, device=DEVICE, track_memory=True)
    results.append(r)
    del encoder
    cleanup()
