# Ralph Loop Iteration 56 - Optimize convergence
# GOALS: val_loss < 1.5, training_time < 100s
# Strategy: 1950 iterations, lower dropout, larger batch for stability

import torch
torch.set_float32_matmul_precision('medium')

# Data - larger batch for better gradients
dataset = "shakespeare_char"
batch_size = 72  # Increased from 64 for more stable gradients
block_size = 128
num_workers = 4

# Model - 7.43M
model_type = "neuromanifold"
n_layer = 6
n_head = 5
n_embd = 320
dropout = 0.05  # Lower dropout for faster convergence (was 0.1)
bias = False

# NeuroManifold features - disabled for speed
use_sdr = False
use_kan = False
use_mhc = False
use_full_mhc = False

# FHN - DISABLED (enables Flash Attention fast path)
n_fhn_steps = 0
fhn_threshold = 0.5
fhn_tau = 12.5
use_fhn_imex = True
use_fhn_partitioning = False
use_fhn_fused = False

# Skip expensive computations
skip_manifold_spectral = True

# Training - 1950 iterations
max_iters = 1950
gradient_accumulation_steps = 1
learning_rate = 2.2e-3   # Slightly higher for faster convergence
min_lr = 1e-4
weight_decay = 0.1
warmup_iters = 50        # Shorter warmup
lr_decay_iters = 1950
grad_clip = 1.0

# Early stopping
early_stopping_patience = 0

# Eval/logging
eval_interval = 650
log_interval = 325
eval_iters = 5
sample_interval = 0

# Output
out_dir = "out-ralph-iter56"
save_checkpoints = False

# Hardware
devices = 1
precision = "bf16-mixed"
compile_model = True

# Logging
wandb_log = False
